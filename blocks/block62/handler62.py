...
def loads(message, bot, aa):
    if message.text == "◀️ Назад":
        ...
        return
    bot.delete_message(message.chat.id, aa) 
    createdb.exdb(message.text, 72, message.chat.id)
    handler63.block4_40(message, bot)
...
def block4_40(message: Message, bot):
    ...
    markup.add(nazad)
    a = bot.send_message(message.chat.id, "<b>🔵 На участке есть уклон?</b>",
                         parse_mode="HTML",
                         reply_markup=markup)
