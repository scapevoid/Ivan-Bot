...
def loads(message, bot, aa):
    ...
    bot.delete_message(message.chat.id, aa) 
    createdb.exdb(message.text, 55, message.chat.id)
...
def block4_40(message: Message, bot):
    ...
    markup.add(nazad)
    a = bot.send_message(message.chat.id, "<b>🔵 Пожалуйста в ответном сообщении укажите ориентировочную площадь котельной</b> \n\n"
