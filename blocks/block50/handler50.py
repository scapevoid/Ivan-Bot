...
from ..block51 import handler51
from ..block49 import handler49
import time
import createdb
def loads(message, bot, aa):
    if message.text == "◀️ Назад":
        handler49.block4_40(message, bot)
        bot.delete_message(message.chat.id, aa) 
        return

    bot.delete_message(message.chat.id, aa) 
    createdb.exdb(message.text, 56, message.chat.id)
    handler51.block4_40(message, bot)
...
def block4_40(message: Message, bot):
    ...
    markup.add(nazad)
    a = bot.send_message(message.chat.id, "<b>🔵 Пожалуйста в ответном сообщении укажите ориентировочную площадь хозблока внутри дома</b> \n\n"
