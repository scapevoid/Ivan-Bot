...
from ..block30 import handler30
from ..block28 import handler28
import time
import createdb
def loads(message, bot, aaaa):
    if message.text == "▶️ Пропустить":
        createdb.exdb("NULL", 33, message.chat.id)
        handler30.block4_30(message, bot)
        bot.delete_message(message.chat.id, aaaa)
        return
    if message.text == "◀️ Назад":
        bot.delete_message(message.chat.id, aaaa)
        handler28.block4_28(message, bot)
        return
    bot.delete_message(message.chat.id, aaaa)
    createdb.exdb(message.text, 33, message.chat.id)
    handler30.block4_30(message, bot)
...
def block4_28(message: Message, bot):
    ...
    a = bot.send_message(
            message.chat.id,
            "<b>🔵Пожалуйста, в ответном сообщении расскажите нам о своих пожеланиях к зоне мастер-спальни.</b>\n\n"
