...
def loads(message, bot, aa):
    if message.text == "▶️ Пропустить":
        createdb.exdb("NULL", 63, message.chat.id)
        infohandler10_5.infoblock10_5(message, bot)
    ...
    if message.text == "◀️ Назад":
        ...
        return
    bot.delete_message(message.chat.id, aa)
    createdb.exdb(message.text, 63, message.chat.id)
    infohandler10_5.infoblock10_5(message, bot)
...
def block4_53(message: Message, bot):
    ...
    a = bot.send_message(message.chat.id, "<b>🔵 Укажите желаемую высоту потолков помещений в цоколе</b>",
                         parse_mode="HTML",
                         reply_markup=markup)
